...
set_build_vs(TxDb, #mrst{} = Mrst, VS, State) ->
    set_build_vs(TxDb, Mrst#mrst.idx_name, VS, State);

set_build_vs(TxDb, #idx{} = Idx, VS, State) ->
    set_build_vs(TxDb, Idx#idx.ddoc, VS, State);

set_build_vs(TxDb, DDocId, VS, State) ->
    #{
        tx := Tx
    } = TxDb,

    Key = build_vs_key(TxDb, DDocId),
...
get_build_vs(TxDb, #mrst{} = Mrst) ->
    get_build_vs(TxDb, Mrst#mrst.idx_name);
...
% write doc does nothing for deleted docs since this is called from the
% background indexer which doesn't need to remove any deleted docs
write_doc(_TxDb, _Sig, _Views, #{deleted := true}) ->
    ok;

write_doc(TxDb, _Sig, _Views, Doc) ->
    #{
        id := DocId,
        results := Results
    } = Doc,
    add_doc(TxDb, DocId, Results).


add_doc(TxDb, DocId, IdxResults) ->
    lists:foreach(fun (IdxResult) ->
        #{
            ddoc_id := DDocId,
            results := Results
...
seq_key(DbPrefix, Sig) ->
    Key = {?DB_MANGO, Sig, ?MANGO_UPDATE_SEQ},
    erlfdb_tuple:pack(Key, DbPrefix).
